...
def draw_boxes_on_image(image: Image.Image, results:Dict, config_path: str) -> Image.Image:
    ...
    for box, score, class_id in zip(results["boxes"], results["scores"], results["class_ids"]):
        ...
        color = tuple(colors.get(label, [255, 0, 0]))  # Default to red if no color specified
...
def predict(image, conf_threshold, iou_threshold, model_name):
    ...
    img = draw_boxes_on_image(image, result_dict, config_path="app/configs/yolov8n.yaml")
...
iface = gr.Interface(
    ...
    inputs=[
        ...
        gr.components.Dropdown(["yolov8n", "yolov8m"], value="yolov8n", label="Trained Models")
    ],
    ...
    examples=[
        ["/home/forz/Test_deepL/HelmetDetection/experiment/gradio/samples/005304.jpg", 0.25, 0.45],
        ["/home/forz/Test_deepL/HelmetDetection/experiment/gradio/samples/005307.jpg", 0.25, 0.45],
    ]
)
