...
from app.configs.model_config import BoundingBox, YOLOScores, load_config
...
class Yolov8_client:
    ...
    def _predict(self, input_images: np.ndarray) -> torch.tensor:
        inputs = [
            self.protocolTriton.InferInput(
                self.input_name, [*input_images.shape], self.fp
            )
        ]
        ...
        for pred in preds:
            ...
            if len(pred):
                ...
                for box in pred[:, :4].cpu().numpy():
                    boxes.append(
                        BoundingBox(xmin=box[0], ymin=box[1], xmax=box[2], ymax=box[3])
                    )
